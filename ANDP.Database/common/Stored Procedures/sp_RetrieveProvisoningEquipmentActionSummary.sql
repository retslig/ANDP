...
CREATE PROCEDURE [common].[sp_RetrieveProvisoningEquipmentActionSummary] @startDate datetime2, @endDate datetime2, @tenantId uniqueidentifier
AS
BEGIN
    ...
    set @sql = 'WITH Base AS ( ' +
			...
			'Counts AS ( ' +
			'		SELECT ' +
			'			EquipmentName, EquipmentAction, EquipmentResult, COUNT(*) AS Number ' +
			'		FROM Base ' +
			'		GROUP BY EquipmentName, EquipmentAction, EquipmentResult ' +
			') ' +
			' ' +
			'SELECT  ' +
			'	c.EquipmentName, ' +
			...
			'FROM Counts AS c ' +
			'OUTER APPLY ( ' +
			'	SELECT EquipmentName, EquipmentAction, SUM(Number) AS Number ' +
			'	FROM Counts ' +
			'	WHERE c.EquipmentName = EquipmentName AND c.EquipmentAction = EquipmentAction AND c.EquipmentResult = EquipmentResult AND EquipmentResult = ''Success'' ' +
			'	GROUP BY EquipmentName, EquipmentAction ' +
			') AS s ' +
			'OUTER APPLY ( ' +
			...
			'	GROUP BY EquipmentName, EquipmentAction ' +
			') AS f ' +
			'GROUP BY c.EquipmentName, c.EquipmentAction ' 
