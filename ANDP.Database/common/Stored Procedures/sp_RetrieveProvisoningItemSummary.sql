...
CREATE PROCEDURE [common].[sp_RetrieveProvisoningItemSummary] @startDate datetime2, @endDate datetime2, @tenantId uniqueidentifier
AS
BEGIN
    ...
    set @sql = 'WITH Base AS ( ' +
			...
			'Counts AS ( ' +
			'		SELECT ' +
			'			ItemType, Result, COUNT(*) AS Number ' +
			'		FROM Base ' +
			...
			'FROM Counts AS c ' +
			'OUTER APPLY ( ' +
			'	SELECT ItemType, SUM(Number) AS Number ' +
			'	FROM Counts ' +
			'	WHERE c.ItemType = ItemType AND c.Result = Result AND Result = ''Success'' ' +
			'	GROUP BY ItemType ' +
			') AS s ' +
			'OUTER APPLY ( ' +
			'	SELECT ItemType, SUM(Number) AS Number ' +
