...
CREATE TRIGGER [ctc].[T_IUD_ctc_Companies] ON [ctc].[Companies]
...
AS BEGIN
	...
	BEGIN
		...
		(
			...
			INNER JOIN [ctc].[Companies] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [ctc].[Companies]
		SET [Version] = CTE1.FinalVersion
		FROM [ctc].[Companies] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [ctc].[Companies] A ON A.Id = D.Id
