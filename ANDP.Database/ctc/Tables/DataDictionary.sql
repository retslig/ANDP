CREATE TABLE [ctc].[DataDictionary] (
    ...
    CONSTRAINT [FK_ctc_DataDictionary_Companies] FOREIGN KEY ([CompanyId]) REFERENCES [ctc].[Companies] ([Id]),
    CONSTRAINT [FK_ctc_DataDictionary_Equipment] FOREIGN KEY ([EquipmentId]) REFERENCES [ctc].[EquipmentSetup] ([Id])
);
...
CREATE TRIGGER [ctc].[T_IUD_ctc_DataDictionary] ON [ctc].[DataDictionary]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [ctc].[DataDictionaryX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [ctc].[DataDictionary] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [ctc].[DataDictionary]
		SET [Version] = CTE1.FinalVersion
		FROM [ctc].[DataDictionary] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [ctc].[DataDictionary] A ON A.Id = D.Id
