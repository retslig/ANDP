CREATE TABLE [ctc].[EquipmentSetup] (
    ...
    CONSTRAINT [FK_ctc_EquipmentSetup_Companies] FOREIGN KEY ([CompanyId]) REFERENCES [ctc].[Companies] ([Id]),
    CONSTRAINT [FK_ctc_EquipmentSetup_EquipmentConnectionSettings] FOREIGN KEY ([EquipmentConnectionSettingsId]) REFERENCES [ctc].[EquipmentConnectionSettings] ([Id]),
    CONSTRAINT [FK_ctc_EquipmentSetup_EquipmentTypeId] FOREIGN KEY ([EquipmentTypeId]) REFERENCES [ctc].[EquipmentTypeEnum] ([Id])
);


GO

CREATE TRIGGER [ctc].[T_IUD_ctc_EquipmentSetup] ON [ctc].[EquipmentSetup]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [ctc].[EquipmentSetupX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [ctc].[EquipmentSetup] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [ctc].[EquipmentSetup]
		SET [Version] = CTE1.FinalVersion
		FROM [ctc].[EquipmentSetup] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [ctc].[EquipmentSetup] A ON A.Id = D.Id
