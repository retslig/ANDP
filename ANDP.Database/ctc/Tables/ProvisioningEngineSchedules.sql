CREATE TABLE [ctc].[ProvisioningEngineSchedules] (
    ...
    CONSTRAINT [FK_ctc_ProvisioningEngineSchedules_ProvisioningEngineSettings] FOREIGN KEY ([ProvisioningEngineSettingsId]) REFERENCES [ctc].[ProvisioningEngineSettings] ([Id])
);
...
CREATE TRIGGER [ctc].[T_IUD_ctc_ProvisioningEngineSchedules] ON [ctc].[ProvisioningEngineSchedules]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [ctc].[ProvisioningEngineSchedulesX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [ctc].[ProvisioningEngineSchedules] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [ctc].[ProvisioningEngineSchedules]
		SET [Version] = CTE1.FinalVersion
		FROM [ctc].[ProvisioningEngineSchedules] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [ctc].[ProvisioningEngineSchedules] A ON A.Id = D.Id
