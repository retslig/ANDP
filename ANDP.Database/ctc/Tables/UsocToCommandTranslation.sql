CREATE TABLE [ctc].[UsocToCommandTranslation] (
    ...
    CONSTRAINT [FK_ctc_UsocToCommandTranslation_Companies] FOREIGN KEY ([CompanyId]) REFERENCES [ctc].[Companies] ([Id]),
    CONSTRAINT [FK_ctc_UsocToCommandTranslation_Equipment] FOREIGN KEY ([EquipmentId]) REFERENCES [ctc].[EquipmentSetup] ([Id])
);
...
CREATE TRIGGER [ctc].[T_IUD_ctc_UsocToCommandTranslation] ON [ctc].[UsocToCommandTranslation]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [ctc].[UsocToCommandTranslationX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [ctc].[UsocToCommandTranslation] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [ctc].[UsocToCommandTranslation]
		SET [Version] = CTE1.FinalVersion
		FROM [ctc].[UsocToCommandTranslation] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [ctc].[UsocToCommandTranslation] A ON A.Id = D.Id
