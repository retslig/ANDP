...
CREATE TRIGGER [emp].[T_IUD_emp_Companies] ON [emp].[Companies]
...
AS BEGIN
	...
	BEGIN
		...
		(
			...
			INNER JOIN [emp].[Companies] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [emp].[Companies]
		SET [Version] = CTE1.FinalVersion
		FROM [emp].[Companies] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [emp].[Companies] A ON A.Id = D.Id
