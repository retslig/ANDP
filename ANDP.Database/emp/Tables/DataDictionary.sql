CREATE TABLE [emp].[DataDictionary] (
    ...
    CONSTRAINT [FK_emp_DataDictionary_Companies] FOREIGN KEY ([CompanyId]) REFERENCES [emp].[Companies] ([Id]),
    CONSTRAINT [FK_emp_DataDictionary_Equipment] FOREIGN KEY ([EquipmentId]) REFERENCES [emp].[EquipmentSetup] ([Id])
);
...
CREATE TRIGGER [emp].[T_IUD_emp_DataDictionary] ON [emp].[DataDictionary]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [emp].[DataDictionaryX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [emp].[DataDictionary] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [emp].[DataDictionary]
		SET [Version] = CTE1.FinalVersion
		FROM [emp].[DataDictionary] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [emp].[DataDictionary] A ON A.Id = D.Id
...
GO
DISABLE TRIGGER [emp].[T_IUD_emp_DataDictionary]
    ON [emp].[DataDictionary];
