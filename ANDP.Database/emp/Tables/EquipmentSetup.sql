CREATE TABLE [emp].[EquipmentSetup] (
    ...
    CONSTRAINT [FK_emp_EquipmentSetup_Companies] FOREIGN KEY ([CompanyId]) REFERENCES [emp].[Companies] ([Id]),
    CONSTRAINT [FK_emp_EquipmentSetup_EquipmentConnectionSettings] FOREIGN KEY ([EquipmentConnectionSettingsId]) REFERENCES [emp].[EquipmentConnectionSettings] ([Id]),
    CONSTRAINT [FK_emp_EquipmentSetup_EquipmentTypeId] FOREIGN KEY ([EquipmentTypeId]) REFERENCES [emp].[EquipmentTypeEnum] ([Id])
);


GO

CREATE TRIGGER [emp].[T_IUD_emp_EquipmentSetup] ON [emp].[EquipmentSetup]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [emp].[EquipmentSetupX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [emp].[EquipmentSetup] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [emp].[EquipmentSetup]
		SET [Version] = CTE1.FinalVersion
		FROM [emp].[EquipmentSetup] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [emp].[EquipmentSetup] A ON A.Id = D.Id
...
GO
DISABLE TRIGGER [emp].[T_IUD_emp_EquipmentSetup]
    ON [emp].[EquipmentSetup];
