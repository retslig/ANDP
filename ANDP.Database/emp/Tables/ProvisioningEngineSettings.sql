CREATE TABLE [emp].[ProvisioningEngineSettings] (
    ...
    CONSTRAINT [FK_emp_ProvisioningEngineSettings_Companies] FOREIGN KEY ([CompanyId]) REFERENCES [emp].[Companies] ([Id]),
    CONSTRAINT [FK_emp_ProvisioningEngineSettings_StatusType] FOREIGN KEY ([ProvisionByMethodTypeId]) REFERENCES [emp].[ProvisionByMethodTypeEnum] ([Id])
);
...
CREATE TRIGGER [emp].[T_IUD_emp_ProvisioningEngineSettings] ON [emp].[ProvisioningEngineSettings]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [emp].[ProvisioningEngineSettingsX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [emp].[ProvisioningEngineSettings] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [emp].[ProvisioningEngineSettings]
		SET [Version] = CTE1.FinalVersion
		FROM [emp].[ProvisioningEngineSettings] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [emp].[ProvisioningEngineSettings] A ON A.Id = D.Id
