CREATE TABLE [emp].[UsocToCommandTranslation] (
    ...
    CONSTRAINT [FK_emp_UsocToCommandTranslation_Companies] FOREIGN KEY ([CompanyId]) REFERENCES [emp].[Companies] ([Id]),
    CONSTRAINT [FK_emp_UsocToCommandTranslation_Equipment] FOREIGN KEY ([EquipmentId]) REFERENCES [emp].[EquipmentSetup] ([Id])
);
...
CREATE TRIGGER [emp].[T_IUD_emp_UsocToCommandTranslation] ON [emp].[UsocToCommandTranslation]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [emp].[UsocToCommandTranslationX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [emp].[UsocToCommandTranslation] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [emp].[UsocToCommandTranslation]
		SET [Version] = CTE1.FinalVersion
		FROM [emp].[UsocToCommandTranslation] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [emp].[UsocToCommandTranslation] A ON A.Id = D.Id
...
GO
DISABLE TRIGGER [emp].[T_IUD_emp_UsocToCommandTranslation]
    ON [emp].[UsocToCommandTranslation];
