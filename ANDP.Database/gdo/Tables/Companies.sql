...
CREATE TRIGGER [gdo].[T_IUD_gdo_Companies] ON [gdo].[Companies]
...
AS BEGIN
	...
	BEGIN
		...
		(
			...
			INNER JOIN [gdo].[Companies] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [gdo].[Companies]
		SET [Version] = CTE1.FinalVersion
		FROM [gdo].[Companies] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [gdo].[Companies] A ON A.Id = D.Id
