CREATE TABLE [gdo].[DataDictionary] (
    ...
    CONSTRAINT [FK_gdo_DataDictionary_Companies] FOREIGN KEY ([CompanyId]) REFERENCES [gdo].[Companies] ([Id]),
    CONSTRAINT [FK_gdo_DataDictionary_Equipment] FOREIGN KEY ([EquipmentId]) REFERENCES [gdo].[EquipmentSetup] ([Id])
);
...
CREATE TRIGGER [gdo].[T_IUD_gdo_DataDictionary] ON [gdo].[DataDictionary]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [gdo].[DataDictionaryX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [gdo].[DataDictionary] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [gdo].[DataDictionary]
		SET [Version] = CTE1.FinalVersion
		FROM [gdo].[DataDictionary] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [gdo].[DataDictionary] A ON A.Id = D.Id
...
GO
DISABLE TRIGGER [gdo].[T_IUD_gdo_DataDictionary]
    ON [gdo].[DataDictionary];
