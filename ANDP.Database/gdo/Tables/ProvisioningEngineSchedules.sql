CREATE TABLE [gdo].[ProvisioningEngineSchedules] (
    ...
    CONSTRAINT [FK_gdo_ProvisioningEngineSchedules_ProvisioningEngineSettings] FOREIGN KEY ([ProvisioningEngineSettingsId]) REFERENCES [gdo].[ProvisioningEngineSettings] ([Id])
);
...
CREATE TRIGGER [gdo].[T_IUD_gdo_ProvisioningEngineSchedules] ON [gdo].[ProvisioningEngineSchedules]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [gdo].[ProvisioningEngineSchedulesX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [gdo].[ProvisioningEngineSchedules] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [gdo].[ProvisioningEngineSchedules]
		SET [Version] = CTE1.FinalVersion
		FROM [gdo].[ProvisioningEngineSchedules] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [gdo].[ProvisioningEngineSchedules] A ON A.Id = D.Id
