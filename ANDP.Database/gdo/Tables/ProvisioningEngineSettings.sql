CREATE TABLE [gdo].[ProvisioningEngineSettings] (
    ...
    CONSTRAINT [FK_gdo_ProvisioningEngineSettings_Companies] FOREIGN KEY ([CompanyId]) REFERENCES [gdo].[Companies] ([Id]),
    CONSTRAINT [FK_gdo_ProvisioningEngineSettings_StatusType] FOREIGN KEY ([ProvisionByMethodTypeId]) REFERENCES [gdo].[ProvisionByMethodTypeEnum] ([Id])
);
...
CREATE TRIGGER [gdo].[T_IUD_gdo_ProvisioningEngineSettings] ON [gdo].[ProvisioningEngineSettings]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [gdo].[ProvisioningEngineSettingsX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [gdo].[ProvisioningEngineSettings] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [gdo].[ProvisioningEngineSettings]
		SET [Version] = CTE1.FinalVersion
		FROM [gdo].[ProvisioningEngineSettings] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [gdo].[ProvisioningEngineSettings] A ON A.Id = D.Id
