CREATE TABLE [ral].[EquipmentSettings] (
...
CREATE TRIGGER [ral].[T_IUD_ral_EquipmentSettings] ON [ral].[EquipmentSettings]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [ral].[EquipmentSettingsX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [ral].[EquipmentSettings] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [ral].[EquipmentSettings]
		SET [Version] = CTE1.FinalVersion
		FROM [ral].[EquipmentSettings] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [ral].[EquipmentSettings] A ON A.Id = D.Id
