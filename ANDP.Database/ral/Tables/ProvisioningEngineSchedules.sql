CREATE TABLE [ral].[ProvisioningEngineSchedules] (
    ...
    CONSTRAINT [FK_ral_ProvisioningEngineSchedules_ProvisioningEngineSettings] FOREIGN KEY ([ProvisioningEngineSettingsId]) REFERENCES [ral].[ProvisioningEngineSettings] ([Id])
);
...
CREATE TRIGGER [ral].[T_IUD_ral_ProvisioningEngineSchedules] ON [ral].[ProvisioningEngineSchedules]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [ral].[ProvisioningEngineSchedulesX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [ral].[ProvisioningEngineSchedules] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [ral].[ProvisioningEngineSchedules]
		SET [Version] = CTE1.FinalVersion
		FROM [ral].[ProvisioningEngineSchedules] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [ral].[ProvisioningEngineSchedules] A ON A.Id = D.Id
