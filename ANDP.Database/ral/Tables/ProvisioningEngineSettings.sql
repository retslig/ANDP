CREATE TABLE [ral].[ProvisioningEngineSettings] (
    ...
    CONSTRAINT [FK_ral_ProvisioningEngineSettings_Companies] FOREIGN KEY ([CompanyId]) REFERENCES [ral].[Companies] ([Id]),
    CONSTRAINT [FK_ral_ProvisioningEngineSettings_StatusType] FOREIGN KEY ([ProvisionByMethodTypeId]) REFERENCES [ral].[ProvisionByMethodTypeEnum] ([Id])
);
...
CREATE TRIGGER [ral].[T_IUD_ral_ProvisioningEngineSettings] ON [ral].[ProvisioningEngineSettings]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [ral].[ProvisioningEngineSettingsX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [ral].[ProvisioningEngineSettings] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [ral].[ProvisioningEngineSettings]
		SET [Version] = CTE1.FinalVersion
		FROM [ral].[ProvisioningEngineSettings] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [ral].[ProvisioningEngineSettings] A ON A.Id = D.Id
