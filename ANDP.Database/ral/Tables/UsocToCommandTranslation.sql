CREATE TABLE [ral].[UsocToCommandTranslation] (
    ...
    CONSTRAINT [FK_ral_UsocToCommandTranslation_Companies] FOREIGN KEY ([CompanyId]) REFERENCES [ral].[Companies] ([Id]),
    CONSTRAINT [FK_ral_UsocToCommandTranslation_Equipment] FOREIGN KEY ([EquipmentId]) REFERENCES [ral].[EquipmentSetup] ([Id])
);
...
CREATE TRIGGER [ral].[T_IUD_ral_UsocToCommandTranslation] ON [ral].[UsocToCommandTranslation]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [ral].[UsocToCommandTranslationX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [ral].[UsocToCommandTranslation] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [ral].[UsocToCommandTranslation]
		SET [Version] = CTE1.FinalVersion
		FROM [ral].[UsocToCommandTranslation] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [ral].[UsocToCommandTranslation] A ON A.Id = D.Id
...
GO
DISABLE TRIGGER [ral].[T_IUD_ral_UsocToCommandTranslation]
    ON [ral].[UsocToCommandTranslation];
