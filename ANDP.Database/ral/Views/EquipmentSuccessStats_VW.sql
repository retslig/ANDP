...
CREATE VIEW [ral].[EquipmentSuccessStats_VW]
AS
WITH Base AS (
	SELECT 
		o.id AS OrderId, 
		...
		i.ResultMessage
	FROM [ral].[Order] as o
	JOIN  [ral].[Service]  as s on s.OrderId = o.Id
	JOIN  [ral].[Item] as i on i.ServiceId = s.Id
	JOIN ral.Equipment AS e on e.ItemId = i.Id
	JOIN [ral].[EquipmentSetup] AS es on es.Id = e.EquipmentSetupId
	JOIN [ral].[StatusTypeEnum] as st on st.Id = e.StatusTypeId
	JOIN [ral].ActionTypeEnum as at on at.Id = e.ActionTypeId
	JOIN [ral].ItemTypeEnum as it on it.id = i.ItemTypeId
	WHERE
		1 = 1
