...
CREATE TRIGGER [srtc].[T_IUD_srtc_Companies] ON [srtc].[Companies]
...
AS BEGIN
	...
	BEGIN
		...
		(
			...
			INNER JOIN [srtc].[Companies] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [srtc].[Companies]
		SET [Version] = CTE1.FinalVersion
		FROM [srtc].[Companies] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [srtc].[Companies] A ON A.Id = D.Id
