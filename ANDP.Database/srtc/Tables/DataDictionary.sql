CREATE TABLE [srtc].[DataDictionary] (
    ...
    CONSTRAINT [FK_srtc_DataDictionary_Companies] FOREIGN KEY ([CompanyId]) REFERENCES [srtc].[Companies] ([Id]),
    CONSTRAINT [FK_srtc_DataDictionary_Equipment] FOREIGN KEY ([EquipmentId]) REFERENCES [srtc].[EquipmentSetup] ([Id])
);
...
CREATE TRIGGER [srtc].[T_IUD_srtc_DataDictionary] ON [srtc].[DataDictionary]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [srtc].[DataDictionaryX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [srtc].[DataDictionary] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [srtc].[DataDictionary]
		SET [Version] = CTE1.FinalVersion
		FROM [srtc].[DataDictionary] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [srtc].[DataDictionary] A ON A.Id = D.Id
