CREATE TABLE [srtc].[EquipmentSettings] (
...
CREATE TRIGGER [srtc].[T_IUD_srtc_EquipmentSettings] ON [srtc].[EquipmentSettings]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [srtc].[EquipmentSettingsX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [srtc].[EquipmentSettings] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [srtc].[EquipmentSettings]
		SET [Version] = CTE1.FinalVersion
		FROM [srtc].[EquipmentSettings] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [srtc].[EquipmentSettings] A ON A.Id = D.Id
