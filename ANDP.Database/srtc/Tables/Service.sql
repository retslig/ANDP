CREATE TABLE [srtc].[Service] (
    ...
    CONSTRAINT [FK_srtc_Service_ActionType] FOREIGN KEY ([ActionTypeId]) REFERENCES [srtc].[ActionTypeEnum] ([Id]),
    CONSTRAINT [FK_srtc_Service_Order] FOREIGN KEY ([OrderId]) REFERENCES [srtc].[Order] ([Id]),
    CONSTRAINT [FK_srtc_Service_StatusType] FOREIGN KEY ([StatusTypeId]) REFERENCES [srtc].[StatusTypeEnum] ([Id]),
...
CREATE STATISTICS [_emp_dta_stat_1397580017_1_18]
    ON [srtc].[Service]([Id], [Version]);


GO
CREATE STATISTICS [_emp_dta_stat_1397580017_1_3]
    ON [srtc].[Service]([Id], [OrderId]);


GO
CREATE STATISTICS [_emp_dta_stat_1397580017_8_1_18]
    ON [srtc].[Service]([StatusTypeId], [Id], [Version]);


GO
CREATE STATISTICS [_emp_dta_stat_1397580017_9_3_8]
    ON [srtc].[Service]([ActionTypeId], [OrderId], [StatusTypeId]);


GO


CREATE TRIGGER [srtc].[T_IUD_srtc_Service] ON [srtc].[Service]
...
AS BEGIN
	...
	BEGIN
		...
		(
			...
			INNER JOIN [srtc].[Service] A ON A.Id = I.Id
	...
	BEGIN
		...
		(
			...
			INNER JOIN [srtc].[Service] A ON A.Id = D.Id
