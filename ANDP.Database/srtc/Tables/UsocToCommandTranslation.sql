CREATE TABLE [srtc].[UsocToCommandTranslation] (
    ...
    CONSTRAINT [FK_srtc_UsocToCommandTranslation_Companies] FOREIGN KEY ([CompanyId]) REFERENCES [srtc].[Companies] ([Id]),
    CONSTRAINT [FK_srtc_UsocToCommandTranslation_Equipment] FOREIGN KEY ([EquipmentId]) REFERENCES [srtc].[EquipmentSetup] ([Id])
);
...
CREATE TRIGGER [srtc].[T_IUD_srtc_UsocToCommandTranslation] ON [srtc].[UsocToCommandTranslation]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [srtc].[UsocToCommandTranslationX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [srtc].[UsocToCommandTranslation] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [srtc].[UsocToCommandTranslation]
		SET [Version] = CTE1.FinalVersion
		FROM [srtc].[UsocToCommandTranslation] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [srtc].[UsocToCommandTranslation] A ON A.Id = D.Id
