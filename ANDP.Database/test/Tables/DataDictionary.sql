CREATE TABLE [test].[DataDictionary] (
    ...
    CONSTRAINT [FK_test_DataDictionary_Companies] FOREIGN KEY ([CompanyId]) REFERENCES [test].[Companies] ([Id]),
    CONSTRAINT [FK_test_DataDictionary_Equipment] FOREIGN KEY ([EquipmentId]) REFERENCES [test].[EquipmentSetup] ([Id])
);
...
CREATE TRIGGER [test].[T_IUD_test_DataDictionary] ON [test].[DataDictionary]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [test].[DataDictionaryX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [test].[DataDictionary] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [test].[DataDictionary]
		SET [Version] = CTE1.FinalVersion
		FROM [test].[DataDictionary] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [test].[DataDictionary] A ON A.Id = D.Id
