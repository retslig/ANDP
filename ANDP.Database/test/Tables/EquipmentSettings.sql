CREATE TABLE [test].[EquipmentSettings] (
...
CREATE TRIGGER [test].[T_IUD_test_EquipmentSettings] ON [test].[EquipmentSettings]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [test].[EquipmentSettingsX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [test].[EquipmentSettings] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [test].[EquipmentSettings]
		SET [Version] = CTE1.FinalVersion
		FROM [test].[EquipmentSettings] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [test].[EquipmentSettings] A ON A.Id = D.Id
