CREATE TABLE [test].[EquipmentSetup] (
    ...
    CONSTRAINT [FK_test_EquipmentSetup_Companies] FOREIGN KEY ([CompanyId]) REFERENCES [test].[Companies] ([Id]),
    CONSTRAINT [FK_test_EquipmentSetup_EquipmentConnectionSettings] FOREIGN KEY ([EquipmentConnectionSettingsId]) REFERENCES [test].[EquipmentConnectionSettings] ([Id]),
    CONSTRAINT [FK_test_EquipmentSetup_EquipmentTypeId] FOREIGN KEY ([EquipmentTypeId]) REFERENCES [test].[EquipmentTypeEnum] ([Id])
);


GO

CREATE TRIGGER [test].[T_IUD_test_EquipmentSetup] ON [test].[EquipmentSetup]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [test].[EquipmentSetupX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [test].[EquipmentSetup] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [test].[EquipmentSetup]
		SET [Version] = CTE1.FinalVersion
		FROM [test].[EquipmentSetup] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [test].[EquipmentSetup] A ON A.Id = D.Id
