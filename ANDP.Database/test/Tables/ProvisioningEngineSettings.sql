CREATE TABLE [test].[ProvisioningEngineSettings] (
    ...
    CONSTRAINT [FK_test_ProvisioningEngineSettings_Companies] FOREIGN KEY ([CompanyId]) REFERENCES [test].[Companies] ([Id]),
    CONSTRAINT [FK_test_ProvisioningEngineSettings_StatusType] FOREIGN KEY ([ProvisionByMethodTypeId]) REFERENCES [test].[ProvisionByMethodTypeEnum] ([Id])
);
...
CREATE TRIGGER [test].[T_IUD_test_ProvisioningEngineSettings] ON [test].[ProvisioningEngineSettings]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [test].[ProvisioningEngineSettingsX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [test].[ProvisioningEngineSettings] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [test].[ProvisioningEngineSettings]
		SET [Version] = CTE1.FinalVersion
		FROM [test].[ProvisioningEngineSettings] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [test].[ProvisioningEngineSettings] A ON A.Id = D.Id
