CREATE TABLE [test].[UsocToCommandTranslation] (
    ...
    CONSTRAINT [FK_test_UsocToCommandTranslation_Companies] FOREIGN KEY ([CompanyId]) REFERENCES [test].[Companies] ([Id]),
    CONSTRAINT [FK_test_UsocToCommandTranslation_Equipment] FOREIGN KEY ([EquipmentId]) REFERENCES [test].[EquipmentSetup] ([Id])
);
...
CREATE TRIGGER [test].[T_IUD_test_UsocToCommandTranslation] ON [test].[UsocToCommandTranslation]
...
AS BEGIN
	...
	BEGIN
		...
		INSERT INTO [test].[UsocToCommandTranslationX] 
	...
	BEGIN
		...
		(
			...
			INNER JOIN [test].[UsocToCommandTranslation] A ON A.Id = I.Id
			GROUP BY I.Id, A.[Version]
		)
		UPDATE [test].[UsocToCommandTranslation]
		SET [Version] = CTE1.FinalVersion
		FROM [test].[UsocToCommandTranslation] Z
	...
	BEGIN
		...
		(
			...
			INNER JOIN [test].[UsocToCommandTranslation] A ON A.Id = D.Id
