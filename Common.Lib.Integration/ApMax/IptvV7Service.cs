...
{
    public class ApmaxIptvV7Service
    {
        ...
        public ApmaxIptvV7Service(EquipmentConnectionSetting setting, ServiceVersions serviceVersions)
        {
            _setting = setting;
            ...
            //http://oasislap7:8731/Design_Time_Addresses/IPTVServiceV7/IPTVService/
            _iptvService = new IPTVServiceClient("WSHttpBinding_IIPTVService1", new EndpointAddress("http://" + _setting.Ip + ":" + _setting.Port + "/Design_Time_Addresses/IPTVServiceV7/IPTVService/"));

            _loginInformation = apAdmin.LoginAdv(setting.CustomString1, setting.Username, setting.Password);
        }

        public IPTVAccountType RetrieveIptvAccountTypesBySubscriberId(string subscriberGuid)
        {
            var iptvAccountType = _iptvService.GetSubscriberAccountBySubscriberId(_loginInformation.LoginToken, subscriberGuid);
            iptvAccountType = FindAndPopulateMissingPackageNamesInIptvAccountTypeClass(new List<IPTVAccountType> { iptvAccountType }).FirstOrDefault();
            return iptvAccountType;
        }
