...
{
    public class VoicemailV3Service
    {
        ...
        public VoicemailV3Service(EquipmentConnectionSetting setting, ServiceVersions serviceVersions)
        {
            _setting = setting;
            _versions = serviceVersions;

            //http://localhost:8731/Design_Time_Addresses/LoginService/ApAdmin/
            var apAdmin = new ApAdminClient("WSHttpBinding_IApAdmin", new EndpointAddress("http://" + _setting.Ip + ":" + _setting.Port +
                                    "/Design_Time_Addresses/LoginService/ApAdmin/"));

            //http://localhost:8731/Design_Time_Addresses/VoicemailService/VoicemailV3/
            _voiceMailService = new VoicemailClient("WSHttpBinding_IVoicemail3", new EndpointAddress("http://" + _setting.Ip + ":" + _setting.Port +
                                    "/Design_Time_Addresses/VoicemailService/VoicemailV3/"));
        ...
        public IEnumerable<VoiceMailBoxType> RetrieveVoicemail(string phoneNumber)
        {
            ...
            VoiceMailBoxType[] myVoiceMailBoxTypeArray =
                _voiceMailService.GetVoiceMailBoxesByVoiceMailBoxNumber(_loginInformation.LoginToken, phoneNumber);
        ...
        {
            NotificationCenterInfoType[] myNotificationCenterInfoTypeArray =
                _voiceMailService.GetAllNotificationCenters(_loginInformation.LoginToken);
        ...
        public void CreateVoicemail(string phoneNumber, string mailBoxDescription, string vmPackageName,
            string subscriberName, string subscriberBillingAccountNumber, MailboxType mailboxType,
            string notificationCenterTypeDescription)
        {
            ...
            {
                if (myPackageType.DescriptionField.Trim()
                    .Equals(vmPackageName.Trim(), StringComparison.OrdinalIgnoreCase))
                {
            ...
            {
                throw new Exception(
                    "Could not find this packagte in the ApMax system. Please check that the correct package was sent to this method." +
                    Environment.NewLine);
            }
            ...
            {
                if (myNotificationCenterInfoType.DescriptionField.Trim()
                    .Equals(notificationCenterTypeDescription.Trim(), StringComparison.OrdinalIgnoreCase))
                {
            ...
            _voiceMailService.AddNewVoiceMailBox(_loginInformation.LoginToken, phoneNumber, mailBoxDescription,
                sPackageGuid, mailboxType, iNotificationCenterID, phoneNumber, subscriberType);
        }
        ...
        public void UpdateVoiceMailBoxFull(string phoneNumber, VoiceMailBoxType voiceMailBoxType,
        ...
        public void UpdateVoiceSubMailbox(string phoneNumber, int numberOfSubMailboxes,
            int maxNumberOfSubMailboxesAllowed)
        {
        ...
        public void UpdateVoiceMailBoxType(string phoneNumber, MailboxType mailBoxType, string internetPassword,string internetUserName, bool internetAccess)
        {
            ...
            foreach (var type in myVoiceMailBoxTypeArray[0].NotificationListField)
            {
                if (type.AddressField.Equals(phoneNumber))
                {
                    ...
                    //Because we need to send the centerid.
                    foreach (
                        NotificationCenterInfoType myNotificationCenterInfoType in myNotificationCenterInfoTypeArray)
                    {
            ...
            foreach (PackageType myPackageType in myPackageTypeArray)
            {
                if (myPackageType.GuidField.Equals(myVoiceMailBoxTypeArray[0].OptionsPackageField,
            ...
            CreateVoicemail(phoneNumber, myVoiceMailBoxTypeArray[0].DescriptionField, sVmPackageName,
                myVoiceMailBoxTypeArray[0].SubscriberListField[0].SubscriberNameField, subscriberBillingAccountNumber,
                mailBoxType, sNotificationCenter);

            if (myVoiceMailBoxTypeArray[0].ChildListField.Length > 0)
            {
                UpdateVoiceSubMailbox(phoneNumber, myVoiceMailBoxTypeArray[0].ChildListField.Length,
                    (int) myVoiceMailBoxTypeArray[0].MaxSubmailboxesField);
            }
        ...
        public void DeleteVoiceMailBox(string phoneNumber, bool deleteSubscriber)
        {
            ...
            if (phoneNumber.Length != 0)
            {
                ...
                {
                    _voiceMailService.DeleteVoiceMailBox(_loginInformation.LoginToken,
                        myVoiceMailBoxTypeArray[0].IdField);
                    ...
                    if (deleteSubscriber)
                    {
                        ...
                        switch (subscriberVersion)
                        {
                            ...
                            case 4:
                                ...
                                break;
                            default:
                                throw new NotImplementedException("Subscriber Service version " + subscriberVersion +
                                                                  " is not implemented.");
                        }
                    }
        ...
        public void DeleteVoiceMailBoxInternetAccess(string phoneNumber)
        {
            if (phoneNumber.Length != 0)
            {
                ...
                _voiceMailService.UpdateVoiceMailBox(_loginInformation.LoginToken, myVoiceMailBoxTypeArray[0],
                    new InternetAccessType {ServiceEnabled = false});
        ...
        public void ReassignVmBoxNumber(string oldPhoneNumber, string newPhoneNumber, string mailboxDescription,
            string subscriberName, string internetPassword, string internetUserName,
            bool internetAccess, bool deleteSubscriber)
        {
            ...
            //This will get the current mailbox info before we delete the box.
            VoiceMailBoxType[] myVoiceMailBoxTypeArray =
                _voiceMailService.GetVoiceMailBoxesByVoiceMailBoxNumber(_loginInformation.LoginToken, oldPhoneNumber);
            ...
            CreateVoicemail(newPhoneNumber, mailboxDescription, sVmPackageName,
                myVoiceMailBoxTypeArray[0].SubscriberListField[0].SubscriberNameField, subscriberBillingAccountNumber,
                myVoiceMailBoxTypeArray[0].MailboxTypeField.HasValue
                    ? myVoiceMailBoxTypeArray[0].MailboxTypeField.Value
                    : new MailboxType(), sNotificationCenter);

            if (myVoiceMailBoxTypeArray[0].ChildListField.Length > 0)
            {
                UpdateVoiceSubMailbox(newPhoneNumber, myVoiceMailBoxTypeArray[0].ChildListField.Length,
                    (int) myVoiceMailBoxTypeArray[0].MaxSubmailboxesField);
            }
        ...
        public void AddOutDialNumber(string phoneNumber, string outDialPhoneNumber, string outDialRoutingNumber)
        {
            ...
            _voiceMailService.AddNewChildMailBox(_loginInformation.LoginToken, myVoiceMailBoxTypeArray[0].IdField,
                new ChildInfoType
                {
        ...
        public void DeleteOutDialNumber(string phoneNumber, string outDialPhoneNumber)
        {
            ...
            foreach (ChildInfoType myChild in myVoiceMailBoxTypeArray[0].ChildListField)
            {
                ...
                {
                    _voiceMailService.DeleteChildMailBox(_loginInformation.LoginToken,
                        myVoiceMailBoxTypeArray[0].IdField, myChild);
                    break;
                }
            }
        }

        public void AddVoiceMailBoxInternetAccess(string phoneNumber, string emailAddress, string internetPassword,
            string internetUserName)
        {
            if (phoneNumber.Length != 0)
            {
                ...
                {
                    throw new Exception(
                        "Unable to determine notification center from TypeField \"NotificationCenterType.typeemail\".");
                }
                ...
                switch (_versions.Subscriber)
                {
                    case 3:
                        ...
                        SubscriberV3.SubscriberType mySubscriberType =
                            apMaxSubscriberService.RetrieveSubscriberByDefaultPhoneNumber(phoneNumber);
                        ...
                        else
                        {
                            ...
                            if (internetUserName == mySubscriberInternetAccessType.UserName)
                            {
                                ...
                                _voiceMailService.UpdateVoiceMailBox(_loginInformation.LoginToken,
                                    myVoiceMailBoxTypeArray[0], new InternetAccessType {ServiceEnabled = true});
        ...
        private static List<int> GetAvailableDigitFieldsFromChildList(IEnumerable<ChildInfoType> myChildInfoTypeArray,
            int iMaxNumberOfSubMailboxesAllowed)
        {
        ...
        public static class ApMaxVoicemailV3ExtensionClass
        {
            public static ChildInfoType GetChildInfoByHighestDigitField(ChildInfoType[] myChildInfoTypeArray)
            {
                ChildInfoType CurrentChildInfoType = new ChildInfoType() {DigitField = "-1"};
