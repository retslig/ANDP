...
namespace Thinktecture.IdentityManager.MembershipReboot
{
        ...
        where TAccount : UserAccount
    {
        ...
        public Task<UserManagerResult<CreateResult>> CreateUserAsync(string username, string password, string tenant)
        {
            try
            {
                UserAccount acct;
                if (this.userAccountService.Configuration.EmailIsUsername)
                {
                    acct = this.userAccountService.CreateAccount(tenant, null, password, username);
                }
                else
                {
                    acct = this.userAccountService.CreateAccount(tenant, username, password, Guid.NewGuid() + "@nothing.com");
                }
